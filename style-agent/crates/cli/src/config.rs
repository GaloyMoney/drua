...
impl Config {
    ...
    pub fn db_path(&self) -> PathBuf {
        self.data_dir().join("style-agent.db")
    }
    ...
    /// Bind address, overridden by `STYLE_AGENT_PORT` env var.
    pub fn bind_addr(&self) -> String {
        if let Ok(port) = std::env::var("STYLE_AGENT_PORT") {
...
/// Load config from the default path or `STYLE_AGENT_CONFIG` env var.
...
fn config_path() -> PathBuf {
    if let Ok(p) = std::env::var("STYLE_AGENT_CONFIG") {
