...
/// practice — `Library::search_global` drops both before fusion (the
/// authz join through `Space.authorized_workspaces` for space files
/// is still pending). Default to Note as a defensive fallback.
