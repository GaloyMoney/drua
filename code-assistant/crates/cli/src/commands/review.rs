...
use code_assistant_core::label_store::{load_audit_suspects, LabelStore};
use code_assistant_core::review_session::{
...
pub fn run(config: &Config, confused: bool) -> anyhow::Result<()> {
    ...
    if total == 0 {
        eprintln!(
            "No chunks found. Run `code-assistant bootstrap` or `code-assistant index` first."
        );
        return Ok(());
    }
